...
class UserStoreController extends Controller
{
    ...
    {
        if ($error = $this->authorize('store-manage')) {
            return $error;
        }
        $users = User::where('role', 6)->get();
        return view('admin.user.store.index', compact('users'));
    }

    public function create()
    {
        if ($error = $this->authorize('store-add')) {
            return $error;
        }
        return view('admin.user.store.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        // Tmm So Id
        $getTmmId = User::where('role', 6)->count() + 1;
        ...
        try {
            ...
            return redirect()->route('company-store.index');
        } catch (\Exception $ex) {
            DB::rollBack();
    ...
    public function show($id)
    {
        ...
        return view('admin.user.store.show', compact('customers'));
    }

    // Edit
    public function edit($id)
    {
        if ($error = $this->authorize('store-edit')) {
            return $error;
        }
        $user = User::find($id);
        $userFiles = UserFile::where('user_id', $id)->get();
        return view('admin.user.store.edit', compact('user', 'userFiles'));
    }
    ...
    {
        if ($error = $this->authorize('store-delete')) {
            return $error;
        }
