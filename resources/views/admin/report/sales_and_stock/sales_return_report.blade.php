                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body" id="printableArea">
                                @php $pageTitle='Sales Return Report' @endphp
                                        ...
                                        <tr style="font-weight: bold; color:#fff" class="bg-info">
                                            ...
                                            <tr>
                                                ...
                                                @foreach($reportSub as $reportSubSub)
                                                    ...
                                                    <td class="text-center">{{ number_format(abs($reportSubSub->amt),2) }}</td>
                                                </tr>
                                                @endforeach
                                                <tr style="font-weight: bold; background: #dfe6e9">
                                                    <td colspan="5" class="text-right">Total: </td>
                                                    <td class="text-center">{{ $reportSub->sum('quantity') }}</td>
                                                    <td class="text-center">{{ number_format(abs($reportSub->sum('amt')),2) }}</td>
                                                </tr>
                                            @endforeach
                                        @endforeach
                                    </tbody>
                                ...
                                <div class="col-md-6">
                                    <table class="table" >
                                        <tr class="bg-secondary text-light" style="font-weight:bold">
                                            <td>Sales: {{ number_format(abs($reportWithDiscount->sum('sales_amt')),2)}}</td>
                                            <td>Discount: {{ number_format($reportWithDiscount->sum('sales_amt')-$reportWithDiscount->sum('net_amt') ,2)}}</td>
                                            <td>Net Amount: {{ number_format(abs($reportWithDiscount->sum('net_amt')),2) }}</td>
                                        </tr>
                                    </table>
                                </div>
