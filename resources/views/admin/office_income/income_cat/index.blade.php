                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="multi-filter-select" class="display table table-striped table-hover" >
                                    ...
                                    <tbody>
                                        @php $x=1; @endphp
                                        @foreach($officeIncomeCats as $officeExpens)
                                        <tr>
                                            <td class="text-center">{{ $x++ }}</td>
                                            <td>{{ $officeExpens->name }}</td>
                                            <td>
                                                <div class="form-button-action">
                                                    <span class="btn btn-link btn-success btn-lg addSub" data-toggle="modal" data-target="#add-sub" data-id="{{$officeExpens->id}}" data-name="{{$officeExpens->name}}"><i class="fas fa-plus"></i></span>
                                                    <span class="btn btn-link btn-primary edit" data-toggle="modal" data-target="#editModal" data-url="{{route('office-income-cat.update', $officeExpens->id)}}" data-id="{{$officeExpens->id}}" data-name="{{$officeExpens->name}}"><i class="fa fa-edit"></i></span>
                                                    <form action="{{ route('office-income-cat.destroy', $officeExpens->id) }}" style="display: initial;" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                        ...
                                        @if ($officeExpens->subCat->count() > 0)
                                            @php $xx=1; @endphp
                                            @foreach($officeExpens->subCat as $subofficeIncome)
                                            <tr>
                                                <td class="text-right">{{ $xx++ }}</td>
                                                <td>{{ $subofficeIncome->name }}</td>
                                                <td>
                                                    <div class="form-button-action">
                                                        <span class="btn btn-link btn-success btn-lg addSubEdit" data-toggle="modal" data-url="{{route('officeIncome.subCatEdit', $subofficeIncome->id)}}" data-target="#add-sub-edit" data-id="{{$subofficeIncome->id}}" data-name="{{$subofficeIncome->name}}" data-parent_name="{{$subofficeIncome->subCatName->name}}"><i class="fas fa-edit"></i></span>
                                                    <form action="{{ route('office-income-cat.destroy', $subofficeIncome->id) }}" style="display: initial;" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" title="Delete" class="btn btn-link btn-danger" onclick="return confirm('Are you sure?')">
                                                                <i class="fa fa-times"></i>
  ...
  <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      ...
      <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
              ...
              <form action="{{ route('office-income-cat.store')}}" method="post" onSubmit="return validate()">
...
<div class="modal fade" id="add-sub" tabindex="-1" role="dialog" aria-labelledby="add-subLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <form action="{{ route('officeIncome.subCatStore')}}" method="post">
            @csrf
                    ...
                    <div class="row">
                        <div class="form-group col-md-12">
                            ...
                            <select name="parent_id" id="" class="form-control">
                                ...
                                @foreach ($officeIncomeCats as $officeExpens)
                                <option value="{{ $officeExpens->id }}">{{ $officeExpens->name }}</option>
                                @endforeach
                            </select>
                            @error('parent_id')
                                <div class="alert alert-danger">{{ $message }}</div>
