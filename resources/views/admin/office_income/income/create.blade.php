...
@section('content')
@php $p = 'account'; $sm='officeIn'; @endphp
        ...
        <div class="page-inner">
            <div class="page-header">
                <ul class="breadcrumbs">
                    ...
                    <li class="nav-item"><a href="{{ route('account-payment.index')}}">Office Income</a></li>
                    <li class="separator"><i class="flaticon-right-arrow"></i></li>
                    <li class="nav-item active">Office Income To</li>
                </ul>
            </div>
                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route('office-income.store')}}" method="post">
                                @csrf
                                ...
                                <div class="row">
                                    ...
                                    {{-- <div class="form-group col-sm-4">
                                        <label for="office_expense_cat_id">Expense Name <span class="t_r">*</span></label>
                                        <select name="office_expense_cat_id" id="" class="form-control" required>
                                            <option selected value disabled>Select</option>
                                            @foreach ($officeExpenseCats as $officeExpenseCat)
                                                <option value="{{ $officeExpenseCat->id }}">{{ $officeExpenseCat->name }}</option>
                                            @endforeach
                                        </select>
                                    </div> --}}

                                    <div class="form-group col-sm-4">
                                        <label for="">Income Type <span class="t_r">*</span></label>
                                        <select name="exp_type" id="in_type" class="form-control" required>
                                            <option selected value disabled>Select</option>
                                            @foreach ($officeIncomes as $officeIncome)
                                            <option value="{{ $officeIncome->id }}">{{ $officeIncome->name }}</option>
                                            @endforeach
                                    ...
                                    <div class="form-group col-sm-4">
                                        <label for="office_income_cat_id">Income Name <span class="t_r">*</span></label>
                                        <select name="office_expense_cat_id" id="in_name" class="form-control" required></select>
                                    </div>
                                    ...
                                    <div class="form-group col-sm-4">
                                        <label for="account_entry_id">Received By <span class="t_r">*</span></label>
                                ...
                                <div class="row" >
                                    ...
                                    <div class="form-group col-sm-4">
                                        ...
                                        <input type="number" id="amount" name="credit" class="form-control @error('credit') is-invalid @enderror" required>
                                        @error('credit')
                                            <div class="alert alert-danger">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="row">
                                    <div class="form-group col-md-3">
                                        <label for="m_r_date">Received Date</label>
                                    ...
                                    <div class="form-group col-md-3">
                                        <label for="m_r_no">Received Voucher No.</label>
...
<script>
    function btnClick() {
        ...
        if(amount > totalCashCredit){
            alert('Cash Limit Over. Available Balance '+ totalCashCredit)
            return false;
        }
        // if(check !=''){
...
<script>
    // Product Price
    $('#in_type').on('change',function(e) {
            var in_type = $('#in_type').val();
            $.ajax({
                url:'{{ route("get.incomeCat") }}',
                type:"get",
                data: {
                    in_type: in_type
                    },
                success:function (res) {
                    res = $.parseJSON(res);
                    // console.log(res.expName)
                    $('#in_name').html(res.inName);
                }
            })
        });
