            ...
            <div class="page-inner">
                <div class="page-header">
                    <ul class="breadcrumbs">
                        <li class="nav-home">
                            <a href="{{ route('admin.dashboard') }}" title="Dashboard"><i class="flaticon-home"></i></a>
                        </li>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="d-flex align-items-center">
                                    <button type="button" class="btn btn-success btn-round ml-auto" id="p"
                                ...
                                <div class="table-responsive" id="printJS-form">
                                    <table class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            @endphp
                                            <tr>
                                                <th colspan="14">Opening</th>
                                                <th class="text-right">{{ $openingBl }}</th>
                                            </tr>
                                            @foreach ($invoices as $invoice)
                                                ...
                                                <tr
                                                    class="{{ $invoice->c_status == 1 ? 'text-primary' : '' }} {{ $pAlart != '' ? 'text-danger' : '' }}">
                                                    <td class="text-center">{{ $x++ }}</td>
                                                    <td>{{ $invoice->type == 0 ? bdDate($invoice->created_at) : bdDate($invoice->invoice_date) }}
                                                    </td>
                                                    ...
                                                    <td class="text-right">{{ number_format(abs($invoice->sales_amt), 2) }}
                                                    </td>
                                                    ...
                                                    <td class="text-center">
                                                        {{ number_format($invoice->discount + $pro_dis, 2) }}</td>
                                                    <td class="text-right">{{ number_format($invoice->discount_amt, 2) }}
                                                    </td>
                                                    ...
                                                    @if (isset($invoice->account->m_r_date))
                                                        <td>{{ empty($invoice->account->m_r_date) ? '' : bdDate($invoice->account->m_r_date) }}
                                                        </td>
                                                    @else
                                                        <td>{{ empty($invoice->payment_date) ? '' : bdDate($invoice->payment_date) }}
                                                        </td>
                                                    @endif
                                                    ...
                                                    @else
                                                        <th></th>
                                                    @endif
                                                    <td>{{ $invoice->inv_cancel == 1 ? 'Invoice Cancel' : ($invoice->inv_cancel == 2 ? 'Reinvoice' : '') }}
                                                        {{ $invoice->account->note ?? '' }}</td>
                                                    ...
                                                    <td class="text-right" style="font-weight: bold">
                                                        {{ number_format($openingBl + ($balance += $b), 2) }}</td>
                                                    @if ($invoice->type != 25 && $invoice->invoice_no != 0)
                                                        <td class="text-center no-print">
                                                            <div class="form-button-action">
                                                                <a href="{{ route('salesLedgerBook.ledgerReportEdit', $invoice->id) }}"
                                                                    title="Edit" class="btn btn-link btn-primary">
                                        ...
                                        <tr class="text-right font-weight-bold">
                                            ...
                                            <td>{{ number_format($openingBl + $invoices->sum('net_amt') - $payment, 2) }}
                                            </td>
                                        </tr>
                                    </table>
